      subroutine sfsia(jdiag1,jdiagaz1,val,
     & f,na,maplg,imaplg,
     & x,diag,nbindex,b,
...
c       write(ext,'(i3)') iblk
c       fmat = 'mat_' // trim(adjustl(ext))
c       frhs = 'rhs_' // trim(adjustl(ext))
c       open(21,file=fmat,form='formatted',status='unknown')
c       open(22,file=frhs,form='formatted',status='unknown')
c       k = 0
c       do i=1,neq
c         if(imaplg(i) .eq. 1) k = k+1
c         if(maplg(i).eq.96816 .and. imaplg(i).eq.1) then
c           print *,iblk,'-----96816::: neql =', i
c         endif
c       enddo
c       write(21,*) k
c
