      subroutine Mpartition(filename,kkcom)
...
C.....OPEN id0 file
      open(4,file=filename(4),form='unformatted',status='old')
      read(4) knode, kdgof
...
C
      ...
      knc11=knc11+knc10
      knc12=knc12+knc11
      if (knc12-1.gt.150000000) then
      write(*,*) 'exceed memory of array ia'
      write(*,*) 'memory of ia = 150000000'
      write(*,*) 'memory needed = ',knc12,' in prgram partition'
      stop 55555
      endif
      kcom=kkcom+1
      knd0=jcom(kcom)
      ...
      call partition(kkcom,knode,knode1,kdgof,
     *ncoor,nparts,numblk,kelem,max_lknode,max_lknode3,
     *max_lkelem,max_lnum,maxa,maxt,numtyp,material,
     *mdivx,mdivy,mdivz,isignpart,ia(kna0),ia(kna1),
     *ia(kna2),ia(kna3),ia(kna4),ib(knb0),ib(knb1),
     *ib(knb2),ia(knc0),ia(knc1),ia(knc2),ia(knc3),
     *ia(knc4),ia(knc5),ia(knc6),ia(knc7),ia(knc8),
     *ia(knc9),ia(knc10),ia(knc11),jc(knd0),jc(knd1),
     *jc(knd2),jc(knd3),jc(knd4),jc(knd5),jc(knd6),
     *ib(kne0),ib(kne1),ib(kne2),
     *filename)
 
      return
      end
      subroutine partition(kkcom,knode,knode1,kdgof,
     *ncoor,nparts,numblk,kelem,max_lknode,max_lknode3,
     *max_lkelem,max_lnum,maxa,maxt,numtyp,material,
     *mdivx,mdivy,mdivz,isignpart,naj,nap,
     *mnode1,nnode1,node,numcol,lm,na,
     *mglnod,nb_elm,node_iblk,mlgelm_no,num_iblk,mnodei,
     *nnodei,idnode,kelem_iblk,nod_ord,coor,id,
     *knode_iblk,mlgnod,iorder,n_update,mnode,nnode,
     *iord_nod,coor_iblk,itemp,iord,
     *filename)
      implicit real*8 (a-h,o-z)
...
c=====================================================================
      ...
      dimension IDNODE(knode), id(knode,kdgof)
      ...
      CHARACTER*1 MATERIAL
      character*22 fname,GraphFile,ext
...
700        CONTINUE
           NUMEL=NUMEL+1
           CALL partition_1(knode,NUMCOL,NAP,NAJ,L,LM,JNA)
           IF (JNA.GT.MAXT) THEN
...
1000      continue
          kelem=kelem+num*nod
2000    CONTINUE
        CALL partition_2(knode,NAP,NAJ,NUMCOL,NA,LM)
...
C.... READ ID0 file
      rewind(4)
      read(4) knode,kdgof,((id(i,j),j=1,kdgof),i=1,knode)
      close(4)
 
C.... Proccess Master and Slave Points
      do i=1,knode
        do j=1,kdgof
          N=id(i,j)
          if( N .lt. -1) then
      ...
      write(21) (dble(idnode(i)),i=1,knode)
      close(21)
...
c.....open time file
      ...
      close(44)
C...  Save mlgelm_no to file
      write(ext,'(i5)') kkcom
      fname='mlgelm_no_' // trim(adjustl(ext))
      open(21,file=fname,form='formatted',status='unknown')
      write(21,*) numblk, numtyp
      do iblk=1,numblk
        ie0=0
        do ityp=1,numtyp
          num = mnodei(iblk,ityp)
          write(21,*) num
          write(21,*) (mlgelm_no(iblk,ie+ie0),ie=1,num)
          ie0 = ie0+num
        enddo
      enddo
      close(21)
...
c----------------------------------------------------
      ...
      SUBROUTINE partition_1(NEQ,NUMCOL,NAP,NAJ,ND,LM,JNA)
      IMPLICIT REAL*8 (A-H,O-Z)
...
400   CONTINUE
      ...
      SUBROUTINE partition_2(NEQ,NAP,NAJ,NUMCOL,NA,LMI)
      IMPLICIT REAL*8 (A-H,O-Z)
...
C       WRITE(*,*) '-----------------'
      ...
      subroutine partition_4(kdgof,knode,id)
